...
\begin{frame}{IoT Security in General} % talk about security challenges which arise with iot
	...
	{
		\note{NETWORK SIZE -> keep energy consumption low -> limit in computation and energy capabilities --> skip authentication, encryption \\
...
\begin{frame}{(Covert) Communication With Lights}
	\note{change between two brightnesses at high rate --> choose two close brightnesses \\
		Hue 255 levels}
...
\begin{frame}{Controlling the Light}
	\note{mention limits of bridge \\
		restrictionss on rate of commands sent in system \\
		brightness increased incrementally -> avoid sharp changes --> thus no phase shifts}
...
\begin{frame}{Overview}
	\note{Arduino setup lght sensor + power supply \\
		sensor frequency output interpreted by picoscope \\
		pytho script set up pico + read out signal + plot to check}
	% Diagramm:
