...
%Go more into detail func extend attack on IoT bulb \\
%Look at results of Ronen/Shamir
...
\begin{frame}{How (smart) LEDs Work}
    \only<1> {
    \note{ \\
        Brightness changes using controller
    }
    ...
    \only<3> {
    \note{ \\
        PU interprets commands -> controls LEDs modulating PW \\
        \\
        Mod. signal sent to driver -> turn on off -> smooth changes 
    }
    ...
    \only<4> {
        ...
        \note{ \\
            Here: see procedure of changing duty cycle
        } 
        \begin{figure}
            \centering
...
% description based on LimitlessLED
    ...
    \only<3> {
    \note{ \\
        One Possibility: sniff setup -> sometimes vulnerable
    }
        \centering
...
\note{ \\ 
        ...
        Send cmd series \\
        \\
        Causes smooth decrease in duty cycle -> inc back
    }
...
\begin{frame}{Decoding: Light Sensor Signal Analysis}
    ...
    \only<3> {
    \note{ \\
        Frequence output to Arduino -> count edges
    }
        \centering
...
\note{ \\
        Attack based on LimitLESS -> not sure if fixed in new API \\
        Philips works different -> undocumented API cmd, not sure if fixed in new bulb \\
        \\
        tend to Philips -> much more brightness levels -> not that easy to detect changes -> fast flickering more easy if changes fixed ?!
    }
